...
if($id)
{
$sql_notices="SELECT * FROM Notices WHERE id=$id";
$notices_details=$conn->query($sql_notices);
if($notices_details->num_rows===1)
{
  $notice=$notices_details->fetch_assoc();
}
}
    ...
    <div class="row">
      <div class="col-md-8">
        <ul class="list-inline custom-breadcrumb">
          <li class="list-inline-item"><a class="h2 text-primary font-secondary" href="notice.php">Notice</a></li>
        ...
        <div class="d-flex">
          ...
          <div class="text-center mr-4">
            <div class="p-4 bg-primary text-white">
              <span class="h2 d-block"><?= date('d', strtotime($notice['notice_date'])) ?></span>
              <?= strtoupper(date('M,Y', strtotime($notice['notice_date']))) ?>
            </div>
          </div>

          <!-- Notice Content -->
          <div>
            <h3 class="mb-3"><?= htmlspecialchars($notice['title']) ?></h3>
            <p class="mb-2"><?= nl2br(htmlspecialchars($notice['description'])) ?></p>

            <!-- Publisher Info -->
            <p class="text-muted mt-3">
              <strong>Published by:</strong> <?= htmlspecialchars($notice['publisher_name']) ?>
            </p>

            <!-- Optional Download Button (if needed for attachments) -->
            <!-- <a href="path/to/file" class="btn btn-primary">Download</a> -->
          </div>
        </div>
      </div>
    </div>
