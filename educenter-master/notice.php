            ...
            <?php while($row = $result->fetch_assoc()): ?>
              <li class="d-md-table mb-4 w-100 border-bottom hover-shadow">
                <div class="d-md-table-cell text-center p-4 bg-primary text-white mb-4 mb-md-0">
                  <?php 
                    $date = date('d', strtotime($row['notice_date']));
                    $monthYear = strtoupper(date('M,Y', strtotime($row['notice_date'])));
                  ?>
                  <span class="h2 d-block"><?= $date ?></span> <?= $monthYear ?>
                </div>
                <div class="d-md-table-cell px-4 vertical-align-middle mb-4 mb-md-0">
                  <a href="notice-single.php?id=<?= $row['id'] ?>" class="h4 mb-3 d-block">
                    <?= htmlspecialchars($row['title']) ?>
                  </a>
                  <p class="mb-1"><?= htmlspecialchars($row['description']) ?></p>
                  <small class="text-muted">Published by: <?= htmlspecialchars($row['publisher_name']) ?></small>
                </div>
                <div class="d-md-table-cell text-right pr-0 pr-md-4">
                  <a href="notice-single.php?id=<?= $row['id'] ?>" class="btn btn-primary">Read More</a>
                </div>
              </li>
            <?php endwhile; ?>
          <?php else: ?>
